...
#rsync_instead="mkdir /Users/enzhao/suanec/ksp/dispatch/rsyncDir/$last_dir"
#$rsync_instead
#cmd="cp -rv $src_dir/* /Users/enzhao/suanec/ksp/dispatch/rsyncDir/$last_dir"
echo "rsync cmd: $cmd"
$cmd
...
if [[ $srvIP = '10.77.29.68' ]] ; then
	echo "fileMode=local"
	url="http://$srvIP:8080/controlCenter-1.0.0/notify.do?fileMode=local&clusterType=$cluster_type&fileName=$last_dir&account=$user_name&jobName=$job_name&version=$cluster_version&command=$command"
else 
	url="http://$srvIP:8080/controlCenter-1.0.0/notify.do?fileMode=rsync&clusterType=$cluster_type&fileName=$last_dir&account=$user_name&jobName=$job_name&version=$cluster_version&command=$command"
	echo false
fi
